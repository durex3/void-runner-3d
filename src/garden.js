...
import {stompWarning} from './danger-vfx.js';
import {bossGroundTarget,BOSS_RECOVERY} from './boss-combat.js';
...
export class Garden{
 constructor({scene,hero,state,enemies,damage,toast,burst,onBossRecovery=()=>{},visualFactory=createRelic,random=Math.random}){Object.assign(this,{scene,hero,state,enemies,damage,toast,burst,onBossRecovery,visualFactory,random});this.plants=[];this.seeds=[];this.buddies=[];this.effects=[];this.combos=new Set();this.stats={planted:0,kills:0,combos:0,destroyed:0};this.stompTimer=7}
 ...
 stomp(pos,source=null){const obj=stompWarning(pos,4);this.scene.add(obj);this.effects.push({obj,life:1.6,max:1.6,radius:4,stomp:true,source});}
 cancelStomps(source){this.effects=this.effects.filter(f=>{if(f.stomp&&f.source===source){this.scene.remove(f.obj);return false}return true});}
 step(dt,drops,hurt){
  ...
  for(const f of this.effects){if(f.stomp&&f.source&&(f.source.dead||!this.enemies().includes(f.source))){f.life=0;continue;}f.life-=dt;if(f.stomp){f.obj.userData.update(1-f.life/f.max);if(f.life<=0){if(this.hero.position.distanceTo(f.obj.position)<4)hurt(28);for(const p of this.plants)if(p.obj.position.distanceTo(f.obj.position)<4){p.dead=true;this.stats.destroyed++}this.burst(f.obj.position);if(f.source){f.source.recovery=BOSS_RECOVERY;this.onBossRecovery(f.source)}}}else f.obj.scale.setScalar(f.radius*(1+(1-f.life/f.max)*.3))}
  this.effects=this.effects.filter(f=>{if(f.life<=0){this.scene.remove(f.obj);return false}return true});const boss=this.enemies().find(e=>e.type==='boss'&&!e.dead);if(boss){this.stompTimer-=dt;if(this.stompTimer<=0&&!boss.rangedWindup&&!(boss.recovery>0)){this.stompTimer=7;this.stomp(bossGroundTarget(this.hero,boss),boss)}}
 }
}

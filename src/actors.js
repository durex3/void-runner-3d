...
export function hitActor(actor){const r=actor.userData.rig;if(!r||r.dead||r.hit>0)return;r.hit=.22;r.hitAction=action(r,r.layered&&actor.userData.profile?.offhand?'Melee_Block_Hit':'Hit_A',true,r.layered?'upper':'full');r.hitAction?.setEffectiveWeight(.55);r.hitAction?.fadeOut(.22)}

...
test('heavy sword visibly knocks ordinary enemies away and softens the boss response',()=>{const s=setup('Knight',1),enemy=s.enemy(2),boss=s.enemy(2,.8,'boss');s.combat.attack();s.step(.8);assert.ok(enemy.obj.position.z>2.5);assert.ok(boss.obj.position.z<2.3);});
...
test('melee impact exposes the exact hit targets for local feedback',()=>{const s=setup('Knight',1),near=s.enemy(2),side=s.enemy(2,1),rear=s.enemy(-2);s.combat.attack();s.step(.41);assert.deepEqual(s.meleeEvents[0].targets,[near,side]);assert.ok(s.meleeEvents[0].targets.every(target=>target.hp<1000));assert.equal(rear.hp,1000)});
